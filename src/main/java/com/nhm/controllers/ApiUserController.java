...
public class ApiUserController {
    ...
    public ResponseEntity<?> login(@RequestBody UserInfo u) {
        ...
        if (this.userDetailsService.authenticate(u.getUsername(), u.getPassword())) {
            try {
                UserInfo userData = this.userDetailsService.getUserByUsername(u.getUsername());
                String userRole = userData.getUserRole();
                String token = JwtUtils.generateToken(u.getUsername(), List.of(userRole));
    ...
    public ResponseEntity<List<Student>> getStudents() {
        List<Student> students = this.userDetailsService.getUsers(Student.class).stream().collect(Collectors.toList());
