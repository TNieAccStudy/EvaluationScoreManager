...
public class ApiSemesterController {
    ...
    @GetMapping("/semesters/{semesterId}/bulletins")
    @JsonView(DisplayView.Simplify.class)
    public ResponseEntity<List<Bulletin>> getBulletinsBySemesterId(@PathVariable("semesterId") int semesterId) {
        List<Bulletin> activities = this.semesterService.getBulletinsByTermId(semesterId).stream().collect(Collectors.toList());
        return new ResponseEntity<>(activities, HttpStatus.OK);
    }
