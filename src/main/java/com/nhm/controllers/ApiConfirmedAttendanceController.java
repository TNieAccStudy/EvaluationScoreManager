...
public class ApiConfirmedAttendanceController {
    ...
    @PatchMapping(path="/{attendanceId}",
            ...
            produces = MediaType.APPLICATION_JSON_VALUE)
    @JsonView(DisplayView.Public.class)
    public ResponseEntity<ActivityConfirmedAttendance> patialUpdate(@PathVariable("attendanceId") int attendanceId, @RequestPart("data") ActivityConfirmedAttendance attendance, @RequestPart("proofPicture") MultipartFile proofPicture) throws Exception {
        return new ResponseEntity<>(this.attendanceService.addOrUpdate(attendance, proofPicture), HttpStatus.OK);
    }
